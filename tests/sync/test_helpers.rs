...
impl Workspace {
    pub fn new() -> Self {
        ...
        let local_root =
            Utf8PathBuf::from_path_buf(local_tmp.path().to_path_buf()).expect("utf8 local path");
        let remote_root =
            Utf8PathBuf::from_path_buf(remote_tmp.path().to_path_buf()).expect("utf8 remote path");
...
pub struct ScriptedContext {
    pub runner: crate::sync::test_doubles::ScriptedRunner,
...
pub fn build_scripted_context(
    runner: crate::sync::test_doubles::ScriptedRunner,
    label: &str,
) -> ScriptedContext {
    let source_tmp = Arc::new(temp_dir(label));
    let source_path = utf8_path(source_tmp.path().to_path_buf(), "scripted context source path");
...
pub fn scripted_context() -> ScriptedContext {
    build_scripted_context(
        crate::sync::test_doubles::ScriptedRunner::new(),

...
- We consider using **sccache** (Rust’s shared compilation cache) to further
  ...
  that **sccache is designed for sharing caches across ephemeral machines**, as
  described in reference [6]. This is exactly our scenario, but because we have
  a volume per project, we already achieve a similar effect (the volume is
  effectively the “shared disk” between ephemeral VMs for that project).

[6]: https://blog.mozilla.org/ted/2016/11/21/sccache-mozillas-distributed-compiler-cache-now-written-in-rust/
